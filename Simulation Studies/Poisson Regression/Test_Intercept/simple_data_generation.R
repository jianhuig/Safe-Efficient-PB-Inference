...
simple_data_gen <- function(n, p = 2, prop_lab = 0.10, 
  ...
  pred_covs <- cbind(1, as.matrix(inference_data[, paste0("x", predset_ind)]))
  
  if (dist_shift) {
    
    if (dist_shift_type == "x1-only") {
      
      beta_shift <- c(unit_vec((p / 2) + 1), rep(0, p / 2))
      
    } else if (dist_shift_type == "x2-only") {
      
      beta_shift <- c(rep(0, p / 2), unit_vec((p / 2) + 1))
      beta_shift <- beta_shift[c((p / 2) + 1, 1 : (p/2), ((p / 2) + 2) : (p + 1))]
      
    } else if (dist_shift_type == "both") {
      
      beta_shift <- c(unit_vec((p / 2) + 1), unit_vec(p / 2))
