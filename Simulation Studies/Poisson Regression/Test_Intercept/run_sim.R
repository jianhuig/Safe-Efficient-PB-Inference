...
sim_result_list <- foreach(i = 1:n_sim, 
                           ...
                           .packages = c("dplyr", "tidyr", "lmtest", "sandwich")) %dopar% 
  {
    ...
    # Simulate beta_1 and beta_2 from unit sphere S^{d-1} - include the intercept. 
    if (dist_shift & dist_shift_type == "x2-only") {
      
      beta <- c(unit_vec(p / 2), unit_vec((p / 2) + 1))
      beta <- beta[c((p / 2) + 1, 1 : (p/2), ((p / 2) + 2) : (p + 1))]
    
    } else {
        
      beta <- c(unit_vec((p / 2) + 1), unit_vec(p / 2))
